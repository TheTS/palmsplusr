...
When printing the column names of the `palmsplus` dataset, you will notice it contains 23 columns: the original 13 plus the 10 that were added as fields.
...
In a normal workflow, you would add domains before building `palmsplus`. This is because the `palms_build_palmsplus()` function seen above adds additional columns to the `palmsplus` dataset specifying domain membership. The `palmsplus` dataset is them summarized into days. 
...
When looking at the first five rows of the `days` dataset, you will notice that three fields (*duration*, *indoors*, and *outdoors*) are summarized for each domain (*total*, *home* and *transport*). These three fields were summarized for each domain because they were specified as `domain_field = TRUE` when they were created (see [Building palmsplus](#building-palmsplus) above). Note that the *duration* field and the *total* domain are included by default.
...
Building trajectories is very similar to building `palmsplus`. Variables that you wish to calculate for each trajectory can be specified with the `palms_add_trajectoy_field()` function. Notice the `palms_epoch` helper function is used here too, so the output will be in seconds.
...
The last component of building trajectories is adding trip start and end locations. This is useful when you want to identify specific trips, such as trips to work or school. This is done with the function `palms_add_trajectory_location(name, start_criteria, end_criteria)`. 
